...
if ($userQ->count() > 0) {
	...
	if($user->isLoggedIn() && $user->data()->id == $userID)
		{
		$editbio = ' <small><a href="edit_profile.php">Editar Bio</a></small>';
		}
	else
		{
		$editbio = '';
		 ...
		 <div class="container">
				...
				<div class="well">
					<div class="row">
						...
						<h1>Perfil de <?php echouser($userID);?></h1>
							<?php echo $usbio.$editbio;?>
						</div>
					</div>
				</div>
					<?php if (checkMenu(2,$user->data()->id)){?>
					<a class="btn btn-success" href="view_all_users.php" role="button">Todos os usuários</a>
					<?php } ?>
	    </div> <!-- /container --><br />
	</div> <!-- /#page-wrapper -->
