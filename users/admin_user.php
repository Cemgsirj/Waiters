...
if(!empty($_POST)) {
  ...
  else
  {
    ...
    if ($userdetails->fone != $_POST['fone']){
      $fone = Input::get("fone");

      $fields=array('fone'=>$fone);
      $validation->check($_POST,array(
        'fone' => array(
          'display' => 'Telefone',
          'required' => true,
    ...
    if($validation->passed()){
      ...
      logger($user->data()->id,"User Manager","Atualizado telefone de $userdetails->fone para $userdetails->fone do usuário $lname.");
    }else{
          ?>
            <?php if(!$validation->errors()=='') {?><div class="alert alert-danger"><?=display_errors($validation->errors());?></div><?php } ?>
            <?php
        ...
        if ($userdetails->cpf != $_POST['cpf']){
          $cpf = Input::get("cpf");
    
          $fields=array('cpf'=>$cpf);
          $validation->check($_POST,array(
            'cpf' => array(
              'display' => 'Cpf',
              'required' => true,
        ...
        if($validation->passed()){
          $db->update('users',$userId,$fields);
          $successes[] = "Cpf atualizado";
          logger($user->data()->id,"User Manager","Atualizado cpf de $userdetails->cpf para $userdetails->cpf do usuário $lname.");
        }else{
              ?>
                <?php if(!$validation->errors()=='') {?><div class="alert alert-danger"><?=display_errors($validation->errors());?></div><?php } ?>
                <?php
        ...
        if($validation->passed()){
          $db->update('users',$userId,$fields);
          $successes[] = "Rg atualizado";
          logger($user->data()->id,"User Manager","Atualizado rg de $userdetails->rg para $userdetails->rg do usuário $lname.");
        }else{
              ?>
                <?php if(!$validation->errors()=='') {?><div class="alert alert-danger"><?=display_errors($validation->errors());?></div><?php } ?>
                <?php
        ...
        if ($userdetails->nascimento != $_POST['nascimento']){
          $nascimento = Input::get("nascimento");
    
          $fields=array('nascimento'=>implode("-",array_reverse(explode("/",$nascimento))));
          $validation->check($_POST,array(
            'nascimento' => array(
              'display' => 'nascimento',
              'required' => true,
        ...
        if($validation->passed()){
          $db->update('users',$userId,$fields);
          $successes[] = "Nascimento atualizado";
          logger($user->data()->id,"User Manager","Atualizado nascimento de $userdetails->nascimento para $userdetails->nascimento do usuário $lname.");
        }else{
              ?>
                <?php if(!$validation->errors()=='') {?><div class="alert alert-danger"><?=display_errors($validation->errors());?></div><?php } ?>
                <?php
...
<div class="row">
        ...
        <div class="panel-heading">ID do usuário: <?=$userdetails->id?><?php if($act==1) {?> - <?php if($userdetails->email_verified==1) {?> E-mail verificado <input type="hidden" name="email_verified" value="1" /><?php } elseif($userdetails->email_verified==0) {?> E-mail não verificado - <label class="normal"><input type="checkbox" name="email_verified" value="1" /> Verificar</label><?php } else {?>Erro: Sem validação<?php } } ?> <?php if($protectedprof==1) {?><p class="pull-right">Perfil protegido - Edição desligada</p><?php } ?> <?php if(in_array($user->data()->id, $master_account)) {?><p class="pull-right"><label class="normal"><input type="checkbox" name="protected" value="1" <?php if($userdetails->protected==1){?>checked<?php } ?>/> Conta protegida</label></p><?php } ?></div>
        <div class="panel-body">

        <label>Cadastro em: </label> <?=echodatetime($userdetails->join_date)?><br/>

        <label>Último login em: </label> <?php if($userdetails->last_login != 0) {?> <?=echodatetime($userdetails->last_login)?> <?php } else {?> <i>Nunca</i> <?php }?><br/>

        <div class="panel panel-default">
		    <div class="panel-heading">Funções <?php if($protectedprof==1) {?><p class="pull-right">Perfil protegido - Edição desligada</p><?php } ?></div>
		    <div class="panel-body">
		        <center>
		            <div class="btn-group"><button type="button" class="btn btn-warning" data-toggle="modal" data-target="#password">Atualizar senha</button></div>
		            <?php if(file_exists($abs_us_root.$us_url_root.'usersc/includes/admin_user_system_settings.php')){?>
		            <div class="btn-group"><button type="button" class="btn btn-info" data-toggle="modal" data-target="#systems">Configurações</button></div><?php } ?>
		            <div class="btn-group"><button type="button" class="btn btn-primary" data-toggle="modal" data-target="#permissions">Permissões</button></div>
		            <div class="btn-group"><button type="button" class="btn btn-default" data-toggle="modal" data-target="#misc">Outras</button></div>
		        </center>
		    </div>
		</div>
        ...
        <input  class='form-control' type='text' name='fone' value='<?=$userdetails->fone?>' />

        <label>Cpf:</label>
        <input  class='form-control' type='text' name='cpf' value='<?=$userdetails->cpf?>' />

        <label>Rg:</label>
        <input  class='form-control' type='text' name='rg' value='<?=$userdetails->rg?>' />

        <label>Nascimento:</label>
        <input  class='form-control' type='date' name='nascimento' value="<?=$userdetails->nascimento?>" />
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	                  <div class="form-group">
	                        <label>Nova senha (Mínimo de <?=$settings->min_pw?>, e máximo de <?=$settings->max_pw?> caracteres.)</label>
	                  ...
	                  <div class="form-group">
	                        ...
	                        <input class='form-control' type='password' name='confirm' <?php if((!in_array($user->data()->id, $master_account) && in_array($userId, $master_account) || !in_array($user->data()->id, $master_account) && $userdetails->protected==1) && $userId != $user->data()->id) {?>disabled<?php } ?>/>
	                                  ...
	                                  <label><input type="checkbox" name="sendPwReset" id="sendPwReset" /> Enviar e-mail de reset?</label>
	      </div>
	      <div class="modal-footer">
	          <div class="btn-group"><input class='btn btn-primary' type='submit' value='Update' class='submit' /></div>
	         <div class="btn-group"><button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button></div>
	      </div>
	    </div>
	...
	<div id="systems" class="modal fade" role="dialog">
	  <div class="modal-dialog">
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-footer">
	          <div class="btn-group"><input class='btn btn-primary' type='submit' value='Atualizar' class='submit' /></div>
	         <div class="btn-group"><button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button></div>
	      </div>
	    </div>
	...
	<div id="permissions" class="modal fade" role="dialog">
	  <div class="modal-dialog">
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
	        <div class="panel panel-default">
	                <div class="panel-heading">Remover permissões: <?php if($protectedprof==1) {?><p class="pull-right">Perfil protegido - Edição desligada</p><?php } ?></div>
	                <div class="panel-body">
	                <?php
	        ...
	        <div class="panel panel-default">
	                <div class="panel-heading">Adicionar permissões: <?php if($protectedprof==1) {?><p class="pull-right">PROTECTED PROFILE - EDIT DISABLED</p><?php } ?></div>
	                <div class="panel-body">
	                <?php
	                foreach ($permissionData as $v1){
	                if(!in_array($v1->id,$perm_ids)){ ?>
	  ...
	  <div class="modal-dialog">
	    ...
	    <div class="modal-content">
	      ...
	      <div class="modal-body">
		    ...
		    <br /><label> Bloquear?:</label>
		    <select name="active" class="form-control">
		            <option value="1" <?php if ($userdetails->permissions==1){echo "selected='selected'";} else { if(!checkMenu(2,$user->data()->id)){  ?>disabled<?php }} ?>>Não</option>
		            <option value="0" <?php if ($userdetails->permissions==0){echo "selected='selected'";} else { if(!checkMenu(2,$user->data()->id)){  ?>disabled<?php }} ?>>Sim</option>
		    </select>

		                    <label> Forçar troca de senha?:</label>
		    <select name="force_pr" class="form-control">
		            <option <?php if ($userdetails->force_pr==0){echo "selected='selected'";} ?> value="0">Não</option>
		            <option <?php if ($userdetails->force_pr==1){echo "selected='selected'";} ?>value="1">Sim</option>
		    </select>
	      ...
	      <div class="modal-footer">
	        ...
	        <input type="hidden" name="csrf" value="<?=Token::generate();?>" />
	                <div class="pull-right">
	                        <div class="btn-group"><input class='btn btn-primary' type='submit' value='Atualizar' class='submit' /></div>
	                        <div class="btn-group"><a class='btn btn-warning' href="admin_users.php">Cancelar</a></div><br /><Br />
	                </div>
