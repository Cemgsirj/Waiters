...
if(Input::exists()){
  ...
  if(!Token::check($token)){
    include('../usersc/scripts/token_error.php');
        ...
        $cpf = Input::get('cpf');
        $rg = Input::get('rg');
        $nomemae = Input::get('nomemae');
        $nascimento = implode("-",array_reverse(explode("/",Input::get('nascimento'))));
        ...
        if (!$agreement_checkbox){
                $validation->addError(["Por favor leia e aceite os termos!"]);
        }
        ...
        if($settings->recaptcha == 1 || $settings->recaptcha == 2){
                ...
                if($reCaptchaValid || $settings->recaptcha == 0){
                        ...
                        if($act==1) logger($theNewId,"User","Registrado com sucesso. E-mail de verificação enviado.");
                        if($act==0) logger($theNewId,"User","Registrado.");
                }
...
require '../users/views/_join_partner.php';
?>
...
$(document).ready(function(){
    ...
    function check_username_ajax(username){
        ...
        $.post('parsers/existingUsernameCheck.php', {'username': username}, function(response) {
            if (response == 'error') $('#usernameCheck').html('Ocorreu um erro ao verificar o nome de usuário.');
            else if (response == 'taken') { $('#usernameCheck').html('<i class="glyphicon glyphicon-remove" style="color: red; font-size: 12px"></i> Nome de usuário já utilizado.');
            $('#next_button').prop('disabled', true); }
            else if (response == 'valid') { $('#usernameCheck').html('<i class="glyphicon glyphicon-ok" style="color: green; font-size: 12px"></i> Nome de usuário disponível.');
