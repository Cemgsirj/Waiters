          ...
          <div class="col-sm-2">
            ...
            if($myUnread == 0 && $checkTo > 0){
              ...
              <form class="" action="message.php?id=<?php echo $id?>" method="post">
                ...
                <input type="submit" class="btn btn-danger" name="markUnread" value="Marcar como não lida">
              </form>
              <?php
            }
            ?>
        ...
        <ul class="chat">
          ...
          foreach ($messages as $m){
              ...
              if($today==$last_message) { $last_update = "Hoje, "; $last_update .= date("H:i:s",$lastmessage); }
                ...
                <?php }else{ ?>
                  ...
                  <div class="chat-body clearfix">
                    ...
                    <?php if($m->msgfrom = $user->data()->id) {?><p class="pull-right"><?php if($m->msg_read==1) {?><i class="glyphicon glyphicon-check"></i> Read<?php } else { ?><i class="glyphicon glyphicon-unchecked"></i> Entregue<?php } ?></p><?php } ?>
                  </div>
                </li>
            ...
            <ul>
              ...
              <h3>Resposta rápida<a href="#" data-toggle="modal" data-target="#reply"><i class="glyphicon glyphicon-new-window"></i></a></h3>
              <form name="reply_form" action="message.php?id=<?=$id?>" method="post">
                <div align="center">
                  <input type="text" class="form-control" placeholder="Clique aqui ou aperte Alt + R para selecionar esta caixa ou Shift + R para abrir o painel completo!" name="msg_body" id="msg_body" <?php if(($perm < 2 && $settings->msg_blocked_users==0) || ($thread->hidden_from==1 || $thread->hidden_to==1)) {?>disabled<?php } ?>/>
                        ...
                        <form name="reply_form" action="message.php?id=<?=$id?>" method="post">
                          ...
                          <div class="modal-footer">
                            <div class="btn-group">       <input type="hidden" name="csrf" value="<?=$csrf?>" />
                              <input class='btn btn-primary' type='submit' name="reply" value='Responder' class='submit' /></div>
                            </form>
