...
<form class="form-signup" action="<?=$form_action;?>" method="<?=$form_method;?>" id="payment-form">
        ...
        <h3 class="">Entre para o time WaitersApp e comece a ganhar dinheiro!</h2>

        <div class="form-group">

                <?php if($settings->auto_assign_un==0) {?><label>Usuário*:</label>&nbsp;&nbsp;<span id="usernameCheck" class="small"></span>
                <input type="text" class="form-control" id="username" name="username" placeholder="usuário" value="<?php if (!$form_valid && !empty($_POST)){ echo $username;} ?>" required autofocus><?php } ?>


                <label for="fname">Primeiro Nome*</label>
                <input type="text" class="form-control" id="fname" name="fname" placeholder="Primeiro Nome" value="<?php if (!$form_valid && !empty($_POST)){ echo $fname;} ?>" required autofocus>

                <label for="lname">Sobrenome*</label>
                <input type="text" class="form-control" id="lname" name="lname" placeholder="Sobrenome" value="<?php if (!$form_valid && !empty($_POST)){ echo $lname;} ?>" required>

                <?php include('../usersc/scripts/additional_join_partner_form_fields.php'); ?>

                <label for="email">E-mail*</label>
                <input  class="form-control" type="text" name="email" id="email" placeholder="E-mail" value="<?php if (!$form_valid && !empty($_POST)){ echo $email;} ?>" required >

<?php

                $character_range = 'Ter entre '.$settings->min_pw . ' e ' . $settings->max_pw;
...
if ($settings->req_cap == 1){
                ...
                $num_caps_statement = '<span id="caps" class="gray_out_text">Ter pelo menos ' . $num_caps . ' letra maiúscula</span>';
}
...
if ($settings->req_num == 1){
                ...
                $num_numbers_statement = '<span id="number" class="gray_out_text">Ter pelo menos ' . $num_numbers . ' número(s).</span>';
}
                $password_match_statement = '<span id="password_match" class="gray_out_text">Ser digitada duas vezes.</span>';
...
?>
                ...
                <div style="display: inline-block">
                        <label for="password">Digite uma senha* (Entre <?=$settings->min_pw?> e <?=$settings->max_pw?> caracteres)</label>
                        <input  class="form-control" type="password" name="password" id="password" placeholder="Senha" required autocomplete="off" aria-describedby="passwordhelp">

                        <label for="confirm">confirme a senha*</label>
                        <input  type="password" id="confirm" name="confirm" class="form-control" placeholder="Confirme a senha" required autocomplete="off" >
                </div>
...
<?php } ?>
                        ...
                        <br><br>
                        <a class="nounderline" id="password_view_control"><span class="glyphicon glyphicon-eye-open"></span> Mostrar senha</a>
                </div>
                <br><br>

                

                <label for="confirm">Termos e condições de uso do serviço</label>
                <textarea id="agreement" name="agreement" rows="5" class="form-control" disabled ><?php require $abs_us_root.$us_url_root.'usersc/includes/partner_agreement.php';?> </textarea>

                <label><input type="checkbox" id="agreement_checkbox" name="agreement_checkbox"> Marque para aceitar os termos e condições</label>
        </div>
        ...
        <button class="submit btn btn-primary " type="submit" id="next_button"><i class="fa fa-plus-square"></i> Cadastrar</button>
</form><br />
</div>
</div>
