  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
        <div class="modal-header">
            ...
            <h4 class="modal-title" id="notification_type">Novas notificações</h4>
        </div>
        <div id="notificationsModalBody" class="col-lg-12" style="padding: 0px; padding-top: 15px"></div>
        <div class="modal-footer">
            <div class="btn-group btn-block"><button type="button" class="btn btn-block btn-default" data-dismiss="modal">Fechar</button></div>
        </div>
    </div>
...
<script type="text/javascript">
	...
	function displayNotifications(new_all) {
		...
		if(new_all == 'new'){
			...
			$('#notification_type').html('Novas notificações');
		} else {
			notif_to_get = {'new_all':'all'}
			$('#notification_type').html('Todas notificações');
		}
        ...
        $.ajax({
            ...
            error: function() {
                $('#notificationsModalBody').html('<div class="text-center btn-lg btn-danger" style="margin: 15px">Ocorreu um erro ao carregar as notificações.</div>');
            }
        });
...
function dismissNotif(id_array) {
	...
	if(id_array.length > 1){
		var confirm_result = confirm('Tem certeza que quer marcar todas como vistas?');
		if(!confirm_result){
			die();
		}
	}
	...
	$.ajax({
		...
		error: function() {
			$('#notificationsModalBody').prepend('<div id="notificationDismissError" class="text-center btn-lg btn-danger" style="margin: 15px">Ocorreu um erro ao marcas as notificações como vistas.</div>');
			$('#notificationDismissError').delay(3200).fadeOut(300);
		}
	});
}
