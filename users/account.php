...
if(!empty($_POST['uncloak'])){
	if(isset($_SESSION['cloak_to'])){
		...
		Redirect::to('admin_users.php?err=You+are+now+you!');
			...
			Redirect::to('logout.php?err=Algo+deu+errado.+Favor+logar+novamente');
		}
}
...
<div class="row">
	<div class="col-xs-12 col-md-3">
		...
		<p><a href="user_settings.php" class="btn btn-primary">Editar Informações</a></p>
		<p><a class="btn btn-primary " href="profile.php?id=<?=$get_info_id;?>" role="button">Perfil Público</a></p>
		<?php
		if($settings->twofa == 1){
		$twoQ = $db->query("select twoKey from users where id = ? and twoEnabled = 0", [$userdetails->id]);
		if($twoQ->count() > 0){ ?>
			<p><a class="btn btn-primary " href="enable2FA.php" role="button">Habilita autenticação em 2 fatores</a></p>
	...
	<div class="col-xs-12 col-md-9">
		...
		<p>Data de cadastro:<?=$signupdate?></p>
		<p>Quantidade de acessos: <?=$user->data()->logins?></p>
