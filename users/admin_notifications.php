...
if (!empty($_POST)) {
  ...
  if ($action=="read" && isset($_POST['checkbox'])){
    ...
    if ($deletion_count = adminNotifications("read",$deletions,$user->data()->id)){
      $successes[] = "$deletion_count nota(s) marcada(s) como lida(s).";
    }
    else {
  ...
  if ($action=="delete" && isset($_POST['checkbox'])){
    ...
    if ($deletion_count = adminNotifications("delete",$deletions,$user->data()->id)){
      $successes[] = "$deletion_count nota(s) apagada(s).";
    }
    else {
      $errors[] = lang("SQL_ERROR");
    }
...
if(!empty($_POST['send_mass_message'])){
  ...
  $notifications->addNotification($msg,$v1->id);
        logger($user->data()->id,"Nota em massa","Enviar nota a $v1->fname.");
      }

  $successes[] = "Sua nota em massa foi enviada!";
  logger($user->data()->id,"Nota em massa","Nota em massa enviada com sucesso.");
}
}
...
<?php if(!$validation->errors()=='') {?><div class="alert alert-danger"><?=display_errors($validation->errors());?></div><?php } ?>
      ...
      [ Selecionar tudo ]</label><?php } ?>                         <div class="btn-group pull-right"><button type="button" class="btn btn-info" data-toggle="modal" data-target="#composemass"><i class="glyphicon glyphicon-plus"></i> Nova mensagem em massa</button></div>
          ...
          <tbody>
              <?php if($count > 0) {?>
                <?php foreach($notifications as $m){?>
                  ...
                  <td><input type="checkbox" class="maincheck" name="checkbox[<?=$m->id?>]" value="<?=$m->id?>"/> <?=echouser($m->user_id)?>, <?=time2str($m->date_created)?><br /><?=$m->message?>
                    <br />
                    <?php if($m->is_read==1 && $m->is_archived==0) {?><i class="glyphicon glyphicon-check"></i> Read<?php } if($m->is_read==0 && $m->is_archived==0) { ?> <i class="glyphicon glyphicon-unchecked"></i> Entregue<?php } if($m->is_archived==1) { ?><i class="glyphicon glyphicon-remove"></i> Apagada<?php } ?>
                  </td>
                </tr>
            <?php  } } else {?>
                        <td colspan="2"><center><h3>Sem notificações</h3></center></td></tr>
                        <?php } ?>
              </tbody>
            </table></center>
						<?php if($count > 0) {?>
            <table class="table pull-right" width="20%">
              ...
              <select class="form-control" name="action" required>
                ...
                <option value="read">Marcar como lida(s)</option>
  ...
  <div class="modal-dialog">
    ...
    <div class="modal-content">
      <div class="modal-header">
        ...
        <h4 class="modal-title">Nova nota em massa</h4>
      </div>
