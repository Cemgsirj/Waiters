...
if(!empty($_POST)) {
    ...
    if(!Token::check($token)){
        ...
        if ($userdetails->username != $_POST['username'] && ($settings->change_un == 1 || (($settings->change_un == 2) && ($user->data()->un_changed == 0)))){
            ...
            if($validation->passed()){
                ...
                $successes[]="Username updated.";
								logger($user->data()->id,"User","Mudar o nome do usu[ario de ] $userdetails->username para $displayname.");
            }else{
        ...
        if ($userdetails->fname != $_POST['fname']){
            ...
            if($validation->passed()){
                ...
                $successes[]='First name updated.';
								logger($user->data()->id,"User","Mudar nome de $userdetails->fname para $fname.");
            }else{
                //validation did not pass
                foreach ($validation->errors() as $error) {
                    $errors[] = $error;
        ...
        if ($userdetails->lname != $_POST['lname']){
            ...
            if($validation->passed()){
                ...
                $successes[]='Last name updated.';
								logger($user->data()->id,"User","Mudar sobrenome de  $userdetails->lname para $lname.");
            }else{
                //validation did not pass
                foreach ($validation->errors() as $error) {
                    $errors[] = $error;
        ...
        if ($userdetails->email != $_POST['email']){
            ...
            if($validation->passed()){
							if($confemail == $email) {
                if($emailR->email_act==0){$db->update('users',$userId,$fields); $successes[]='Email updated.'; logger($user->data()->id,"User","Mudar e-mail de $userdetails->email para $email."); }
                if($emailR->email_act==1){
                    $db->update('users',$userId,['email_new'=>$email]);
				            ...
				            if(!$email_sent) $errors[] = 'E-mail não enviado, favor enviar uma mensagem ao administrador.';
										else $successes[]="E-mail enviado. Por favor verifique o seu e-mail para validação.";
										if($emailR->email_act==1) logger($user->data()->id,"User","Solicitar mudança de e-mail de $userdetails->email para $email. E-mail de verificação enviado.");
                }
          }
					else $errors[] = "E-mail não confere.";
				 }else{
                //validation did not pass
                foreach ($validation->errors() as $error) {
                    $errors[] = $error;
        ...
        <div class="well">
            <div class="row">
                ...
                <div class="col-xs-12 col-md-10">
                    ...
                    <strong>Para mudar a sua imagem de exibição, </strong><br>Visite <a href="https://en.gravatar.com/">https://en.gravatar.com/</a> e configure uma conta com seu e-mail cadastrado. É rápido e o gravatar é usado em vários sites!<br>
                    ...
                    <form name='updateAccount' action='user_settings.php' method='post'>
                        ...
                        <div class="form-group">
                            ...
                            <?php if (($settings->change_un == 0) || (($settings->change_un == 2) && ($userdetails->un_changed == 1)) ) {?>
															<div class="input-group">
																 ...
																 <span class="input-group-addon"data-toggle="tooltip" title="<?php if($settings->change_un==0) {?>Não é possível alterar o nome de usuário.<?php } if(($settings->change_un == 2) && ($userdetails->un_changed == 1)) {?>Só é permitida a troca de nome de usuário uma vez.<?php } ?>">Porque eu não posso atualizar?</span>
															 </div>
                            <?php }else{ ?>
                        ...
                        <div class="form-group">
                            ...
                            <input class='form-control' type='text' name='email' value='<?=$userdetails->email?>' />
														<?php if(!IS_NULL($userdetails->email_new)) {?><br /><div class="alert alert-danger">
															<p><strong>Atenção</strong> existe uma atualização pendente para o e-mail <?=$userdetails->email_new?>.</p>
															<p>Por favor confirme o seu novo e-mail.</p>
															<p>Caso não tenha recebido ou preciso de outra solicita;cão. Preencha o seu e-mail abaixo e clique e reenviar.</p>
														</div><?php } ?>
                        </div>
	                      ...
	                      <div class="input-group" data-container="body">
	                       ...
	                       <span class="input-group-addon pwpopover" id="addon4" data-container="body" data-toggle="popover" data-placement="top" data-content="Must match the New Password">?</span>
											 ...
											 <div class="form-group">
													 <label>Senha anterior, obrigatório para mudança de e-mail ou senha.</label>
													 <div class="input-group" data-container="body">
                        ...
                        <p><input class='btn btn-primary' type='submit' value='Atualizar' class='submit' /></p>
                        <p><a class="btn btn-info" href="account.php">Cancelar</a></p>

                    </form>
                    <?php
                    if(isset($user->data()->oauth_provider) && $user->data()->oauth_provider != null){
                        echo "<strong>NOTE:</strong> Se você se cadastrou com uma conta do Google/Facebook, você precisa usar o link de esqueci minha senha...";
                    }
                    ?>
                </div>
            </div>
