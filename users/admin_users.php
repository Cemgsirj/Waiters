        ...
        if($validation->passed()) {
          ...
          try {
            ...
            if(isset($_POST['sendEmail'])) {
              ...
              $subject = 'Bem vindo ao '.$settings->site_name;
              $body = email_body('_email_adminUser.php',$params);
              email($to,$subject,$body);
            }
            logger($user->data()->id,"User Manager","Adicionado o usuário $username.");
            ...
            <div class="row">
              ...
              <?php if(in_array($user->data()->id,$master_account)){ ?>
                <form class="" action="admin_users.php" method="post">
                  <label for="">Entre o id do usuário para assumir a conta</label><br>
                  <input type="text" name="to_user" value="">
                  <input type="submit" name="cloak" value="Assumir!" class='btn btn-danger'>
                </form>
              <?php } ?>
              <a class="pull-right" href="#" data-toggle="modal" data-target="#adduser"><i class="glyphicon glyphicon-plus"></i> Adicionar usuário</a>
              <div class="row">
                <div class="col-xs-12">
                  ...
                  <div class="allutable">
                    <table id="paginate" class='table table-hover table-list-search'>
                      <thead>
                        <tr>
                          <th></th><th></th><th>Login</th><th>Nome</th><th>E-mail</th>
                          <th>Último acesso</th><?php if($act==1) {?><th>Verificado</th><?php } ?><th>Status</th>
                        </tr>
                      </thead>
                      <tbody>
                        <?php
                        ...
                        foreach ($userData as $v1) {
                          ...
                          <tr>
                            ...
                            <td><?=$v1->email?></td>
                            <td><?php if($v1->last_login != 0) {?> <?=echodatetime($v1->last_login)?><?php } else {?> <i>Nunca</i> <?php }?></td>
                            <?php if($act==1) {?><td>
              ...
              <div class="modal-dialog">
                <div class="modal-content">
                  <div class="modal-header">
                    ...
                    <h4 class="modal-title">Adicionar usuário STAFF</h4>
                  </div>
                  <div class="modal-body">
                    <form class="form-signup" action="admin_users.php" method="POST" id="payment-form">
                      <div class="panel-body">
                        <?php if($settings->auto_assign_un==0) {?><label>Usuário: </label>&nbsp;&nbsp;<span id="usernameCheck" class="small"></span><input type="text" class="form-control" id="username" name="username" placeholder="usuário" value="<?php if (!$form_valid && !empty($_POST)){ echo $username;} ?>" required><?php } ?>
                          <label>Nome: </label><input type="text" class="form-control" id="fname" name="fname" placeholder="Nome" value="<?php if (!$form_valid && !empty($_POST)){ echo $fname;} ?>" required>
                          <label>Sobrenome: </label><input type="text" class="form-control" id="lname" name="lname" placeholder="Sobrenome" value="<?php if (!$form_valid && !empty($_POST)){ echo $lname;} ?>" required>
                          <label>Telefone: </label><input  class="form-control" type="text" name="fone" id="fone" placeholder="Telefone com DDD" value="<?php if (!$form_valid && !empty($_POST)){ echo $fone;} ?>" required >
                          <label>Email: </label><input  class="form-control" type="text" name="email" id="email" placeholder="Email" value="<?php if (!$form_valid && !empty($_POST)){ echo $email;} ?>" required >
                          <label>Senha: </label>
                          <div class="input-group" data-container="body">
                            ...
                            <input  class="form-control" type="password" name="password" id="password" <?php if($settings->force_pr==1) { ?>value="<?=$random_password?>" readonly<?php } ?> placeholder="Senha" required aria-describedby="passwordhelp">
                          ...
                          <div class="input-group" data-container="body">
                            <span class="input-group-addon password_view_control" id="addon1"><span class="glyphicon glyphicon-eye-open"></span></span>
                            <input  type="password" id="confirm" name="confirm" <?php if($settings->force_pr==1) { ?>value="<?=$random_password?>" readonly<?php } ?> class="form-control" placeholder="Confirme a senha" required >
                            <?php if($settings->force_pr==1) { ?>
                              <span class="input-group-addon" id="addon2"><a class="nounderline pwpopover" data-container="body" data-toggle="popover" data-placement="top" data-content="Em cadastro manual um e-mail com o link para troca de senha é enviado para que o usuário troque a senha por motivos de segurança.">Porque eu não posso editar isto?</a></span>
                            <?php } ?>
                          </div>
                          <label><input type="checkbox" name="sendEmail" id="sendEmail" checked /> Enviar e-mail?</label>
                          <br />
                        </div>
                        <div class="modal-footer">
                          <div class="btn-group">
                            <input type="hidden" name="csrf" value="<?=Token::generate();?>" />
                            <input class='btn btn-primary' type='submit' id="addUser" name="addUser" value='Adicionar' class='submit' /></div>
