          ...
          <div class="row">
            <div class="col-sm-12">
              <div class="row" id="maindiv">
                <div>
                  <h1><center>Mensagens <a href="#" data-toggle="modal" class="nounderline" data-target="#settings"><i class="glyphicon glyphicon-cog"></i></a></center></h1>
                </div>
                <?php if($count > 0) {?><label><input type="checkbox" class="checkAllMsg" />
                  [ check/uncheck todas ]</label><?php } ?>                         <div class="btn-group pull-right"><button type="button" class="btn btn-info" data-toggle="modal" data-target="#composemass"><i class="glyphicon glyphicon-plus"></i> Nova mensagem em massa</button></div>
                                ...
                                if($today==$last_message) { $last_update = "Today, "; $last_update .= date("g:i A",$lastmessage); }
                                  ...
                                  <td class="pull-left">
                                    <h4>
                                      ...
                                      <a class="nounderline" href="admin_message.php?id=<?=$m->id?>">
                                        Atualizada <?=$last_update?> por <?php echouser($m->last_update_by);?>
                                      </a>
                                    </td>
                                    <td>
                                      <span class="pull-left"><i class="glyphicon glyphicon-<?php if($m->archive_from==0) {?>unchecked<?php } else {?>check<?php } ?>"></i> Arquivada por <?=echouser($m->msg_from)?> (f)</span>
                                      <span class="pull-right"><i class="glyphicon glyphicon-<?php if($m->archive_to==0) {?>unchecked<?php } else {?>check<?php } ?>"></i> Arquivada por <?=echouser($m->msg_to)?> (t)</span><br /><hr>
                                      <span class="pull-left"><i class="glyphicon glyphicon-<?php if($m->hidden_from==0) {?>unchecked<?php } else {?>check<?php } ?>"></i> Apagada por <?=echouser($m->msg_from)?> (f)</span>
                                      <span class="pull-right"><i class="glyphicon glyphicon-<?php if($m->hidden_to==0) {?>unchecked<?php } else {?>check<?php } ?>"></i> Apagada por <?=echouser($m->msg_to)?> (t)</span>
                                    </td>
                                  </tr>
                                <?php } } else {?>
                                  <td colspan="2"><center><h3>Sem mensagens</h3></center></td></tr>
                                <?php } ?>
                              </tbody>
                            </table></center>
                            <?php if($count > 0) {?>
                                ...
                                <tr>
                                  <td width="15%">
                                    <select class="form-control" name="action" required>
                                      ...
                                      <option value="archiveto">Arquivar as enviadas</option>
                                      <option value="archivefrom">Arquivar as recebidas</option>
                                      <option value="archive">Arquivar todas</option>
                                      <option value="unarchiveto">Recuperar as enviadas</option>
                                      <option value="unarchivefrom">Recuperar as recebidas</option>
                                      <option value="unarchive">Recuperar todas</option>
                                      <option value="deleteto">Apagar as enviadas</option>
                                      <option value="deletefrom">Apagar as recebidas</option>
                                      <option value="delete">Apagar todas</option>
                                    </select>
                                  </td>
                                  <td width="5%">
                                    <input type="hidden" name="csrf" value="<?=$csrf;?>" />
                                    <input class='btn btn-primary' type='submit' name="admin_messages" value='Aplicar' class='submit' /></td></tr></table>
                                  <?php } ?>
                                </form>
                              ...
                              <div class="modal-dialog">
                                ...
                                <div class="modal-content">
                                  <div class="modal-header">
                                    ...
                                    <h4 class="modal-title">Ajustes de mensagem</h4>
                                  </div>
                                  <div class="modal-body">
                                    <form class="form" id="messageSettings" name='messageSettings' action='admin_messages.php' method='post'>
                                      <div class="form-group">
                                        <label for="msg_notification">Notificar por e-mail?</label>
                                        <select id="msg_notification" class="form-control" name="msg_notification">
                                          <option value="1" <?php if($settings->msg_notification==1) echo 'selected="selected"'; ?> >Habilitado</option>
                                          <option value="0" <?php if($settings->msg_notification==0) echo 'selected="selected"'; ?> >Desabilitado</option>
                                        </select>
                                      </div>

                                      <div class="form-group">
                                        <label for="msg_blocked_users">Permitir mensagem para usuários bloqueados?</label>
                                        <select id="msg_blocked_users" class="form-control" name="msg_blocked_users">
                                          <option value="1" <?php if($settings->msg_blocked_users==1) echo 'selected="selected"'; ?> >Habilitado</option>
                                          <option value="0" <?php if($settings->msg_blocked_users==0) echo 'selected="selected"'; ?> >Desabilitado</option>
                                        </select>
                                      </div>

                                      <div class="form-group">
                                        <label for="msg_default_to">Desligar resposta (messageUser function)?</label>
                                        <select id="msg_default_to" class="form-control" name="msg_default_to">
                                          <option value="1" <?php if($settings->msg_default_to==1) echo 'selected="selected"'; ?> >Sim</option>
                                          <option value="0" <?php if($settings->msg_default_to==0) echo 'selected="selected"'; ?> >Não</option>
                                        </select>
                                      </div>
                                    </div>
                                    <div class="modal-footer">
                                      <div class="btn-group">
                                        <input type="hidden" name="csrf" value="<?=$csrf;?>" />
                                        <input class='btn btn-primary' type='submit' name="messageSettings" value='Atualizar' class='submit' /></div>
                                      </form>
                              ...
                              <div id="composemass" class="modal fade" role="dialog">
                                <div class="modal-dialog">
                                  ...
                                  <div class="modal-content">
                                    <div class="modal-header">
                                      <button type="button" class="close" data-dismiss="modal">&times;</button>
                                      <h4 class="modal-title">Nova mensagem em massa</h4>
                                    </div>
                                    <div class="modal-body">
                                      <form name="create_mass_message" action="admin_messages.php" method="post">
                                        ...
                                        <label><input type="checkbox" name="sendEmail" id="sendEmail" checked /> Envio de e-mail está ligado?</label>
                                        <input type="hidden" name="csrf" value="<?=$csrf;?>" />
                                      </p>
                                      <p>
                                        <br />
                                      ...
                                      <div class="modal-footer">
                                        <div class="btn-group">
                                          <input type="hidden" name="csrf" value="<?=$csrf;?>" />
                                          <input class='btn btn-primary' type='submit' name="send_mass_message" value='Enviar mensagem' class='submit' /></div>
                                        </form>
                                        <div class="btn-group"><button type="button" class="btn btn-default" data-dismiss="modal">Fechar</button></div>
                                      </div>
                                    </div>
