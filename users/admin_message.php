...
<?php if(!$validation->errors()=='') {?><div class="alert alert-danger"><?=display_errors($validation->errors());?></div><?php } ?>
    <div class="row">
      ...
      <div class="col-sm-10 col-sm-offset-1">
        <div class="row">
          <div class="col-sm-10">
            <h2><a href="admin_messages.php"><i class="glyphicon glyphicon-chevron-left"></i></a> <?=$thread ->msg_subject?> - ADMIN</h2>
          </div>
          <div class="col-sm-2">
          </div>
        </div>
        <label><input type="checkbox" class="checkAllMsg" />
        [ Marcar/desmarcar tudo ]</label>
            ...
            if($m->msg_to == $user->data()->id){
              ...
              <div class="chat-body clearfix">
                <div class="header">
                  ...
                  <p class="pull-right"><?php if($m->msg_read==1 && $m->deleted==0) {?><i class="glyphicon glyphicon-check"></i> Ler<?php } if($m->msg_read==0 && $m->deleted==0) { ?><i class="glyphicon glyphicon-unchecked"></i> Entregue<?php } if($m->deleted==1) { ?><i class="glyphicon glyphicon-remove"></i> Apagada<?php } ?></p>
                  <?php if($m->deleted==0) {?><br /><label class="pull-right"><input type="checkbox" class="maincheck" name="delete[<?=$m->id?>]" value="<?=$m->id?>"/> Apagar?</label><?php } ?>
                  <?php if($m->deleted==1) {?><br /><label class="pull-right"><input type="checkbox" class="maincheck" name="undelete[<?=$m->id?>]" value="<?=$m->id?>"/> Desfazer?</label><?php } ?>
                </div>
              </li>
              ...
              <?php }else{ ?>
                ...
                <div class="chat-body clearfix">
                  ...
                  <p class="pull-right"><?php if($m->msg_read==1 && $m->deleted==0) {?><i class="glyphicon glyphicon-check"></i> Ler<?php } if($m->msg_read==0 && $m->deleted==0) { ?><i class="glyphicon glyphicon-unchecked"></i> Entregue<?php } if($m->deleted==1) { ?><i class="glyphicon glyphicon-remove"></i> Apagar<?php } ?></p>
                  <?php if($m->deleted==0) {?><br /><label class="pull-right"><input type="checkbox" class="maincheck" name="delete[<?=$m->id?>]" value="<?=$m->id?>"/> Apagar?</label><?php } ?>
                  <?php if($m->deleted==1) {?><br /><label class="pull-right"><input type="checkbox" class="maincheck" name="undelete[<?=$m->id?>]" value="<?=$m->id?>"/> Desfazer?</label><?php } ?>
                </div>
              </li>
              ...
              <ul>
                ...
                <input class='btn btn-primary pull-right' type='submit' name="action" value='Atualizar' class='submit' /></div><br /></form>
              </div> <!-- /.col --><br />
              </div> <!-- /.row -->
            </div> <!-- /.container -->
          </div> <!-- /.wrapper -->
